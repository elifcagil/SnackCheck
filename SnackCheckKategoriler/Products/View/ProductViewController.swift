...
class ProductViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let gidilecekVC = segue.destination as! ButonDetailViewController
...
extension ProductViewController: UICollectionViewDelegate, UICollectionViewDataSource  {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "urunleritem", for: indexPath) as! ProductCollectionViewCell
        cell.urunbrand.text = urun.product_brand
        cell.urunname.text = urun.product_name
        cell.urunimage.image = UIImage(named: urun.product_image!)
...
extension ProductViewController:UrunHucreCollectionViewCellProtokol{ //collectionviewda tanımladığımız protokolü referans aldık.bu protokolden bize veri gelecek.indexpath verisi gelecek.
    func FavorilereEkle(indexPath: IndexPath) {
        print("helal be sana \(viewModel.urunlerListe[indexPath.item].product_name!) favorilere ekledin sonunda")
...
extension ProductViewController : UISearchBarDelegate{
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchText == "" {
            ...
            arananurunler = (viewModel.urunlerListe.filter { $0.product_name?.lowercased().contains(searchText.lowercased()) ?? false})
